...
def server(input, output, session):
    @render.plot
    def attendance_plot():  
        df = pd.DataFrame("attendance_anonymised.csv")
        ...
        history_df.groupby(history_df['Date'].dt.date)['Attended'].mean().plot(kind="line", grid=True, title="Average Attendance for History Module Over Time", ylabel="Average Attendance", xlabel="Date", figsize=(10,6), marker='o')
        return fig
